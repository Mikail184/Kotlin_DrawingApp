...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        ib_save.setOnClickListener {
            if(isReadStorageAllowed()) {
                getBitmapFromView(fl_drawing_view_container)?.let { it1 -> BitmapAsyncTask(it1).execute() }
            } else {
                requestStoragePermission()
            }
        }
            ...
            if(requestCode == GALLERY) {
                try{
                    if(data!!.data !=null){
                        ...
                        Toast.makeText(this, "Error in parsing the picture or its corrupted", Toast.LENGTH_SHORT).show()
                    }
                }catch (e: Exception) {
                    e.printStackTrace()
                }
    ...
    private inner class BitmapAsyncTask(val mBitmap: Bitmap) : AsyncTask<Any, Void, String> () {
        ...
        override fun onPostExecute(result: String?) {
            ...
            if (result != null) {
                if(!result.isEmpty()) {
                    Toast.makeText(this@MainActivity, "Picture saved $result", Toast.LENGTH_SHORT).show()
                } else {
                    Toast.makeText(this@MainActivity, "Oops something went wrong with saving the file", Toast.LENGTH_SHORT).show()
                }
